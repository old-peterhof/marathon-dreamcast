...
| b68 | **Pause menu redraws the whole screen on every cursor move** — visibly slow, painting top to bottom. Content is correct. | `draw_screen` in dc_screen.cpp does a full-screen software alpha blend for the scrim whenever the pause menu is over the game — `dc_ui_blend(v, 0, 0, v->w, v->h, ink, 230)`, 307,200 pixels on a 200MHz SH4 — and then `SDL_UpdateRect(v, 0, 0, 0, 0)`, a full 600K push. Both happen again for each keypress, because the run loop repaints everything whenever `dirty` is set. **Fix, not yet built:** blend the scrim once on entry and repaint only what changed on later passes — the panel rect and the explainer line — updating just those rectangles. The title, kicker, rule and hint bar never change while a screen is up. Untried caveat: this assumes nothing repaints the framebuffer between cursor moves. If the game view is still being rendered behind the pause menu, the scrim has to be re-applied each pass and the answer is a cached blended surface instead, at 600K of heap. |
