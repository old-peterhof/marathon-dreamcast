...
| b93 | mtxstack | d8210f4 | GLdc: never pop the base matrix / guard empty stack_replace -- fixes the heap smash (identity 1.0 written before the matrix-stack buffer) behind the Loading C3 freeze |
| b94 | loadmenu | 926867c | pause menu gets LOAD GAME (pick any save after death); quit-to-menu confirm uses the DC UI not the black SDL dialog; main-menu save note trimmed so a long level name cannot spill over the menu |
