...
#ifdef DC
		// Read the pad. This loop only ever looked at SDL's event queue, and
		// nothing puts a Dreamcast controller into that queue except
		// dc_input_poll(). So the ten-second hold on the chapter screen could
		// not be skipped from a console at all, and every level load carried the
		// whole delay -- measured at 22.5 seconds for the chapter screen, of
		// which ten are this wait.
		dc_input_poll();
#endif
		SDL_PollEvent(&event);
