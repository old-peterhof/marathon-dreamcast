...
static mem_fd_t fh[MAX_MEM_OPEN];
...
uint32 mem_open(const char *fn, int mode) {
	...
	if (mode & O_DIR) {
		dirfd.idx = 0;
		strcpy(dirfd.name,fn);
		if(dirfd.name[0]) {
			if (dirfd.name[strlen(dirfd.name)-1]!='/') strcat(dirfd.name,"/");
		}
		return (uint32)&dirfd;
	}
	...
	if (tblidx==-1) {
		...
		if (mode & O_TRUNC) {
			tbl[tblidx].size = 0;
			if (tbl[tblidx].data) {
				free(tbl[tblidx].data);
				tbl[tblidx].data = NULL;
			}
		}
	}
	...
	spinlock_lock(&mutex);
	for (i=0; i<MAX_MEM_OPEN && fh[i].tblidx!=-1; i++) ;
	spinlock_unlock(&mutex);
	if (i >= MAX_MEM_OPEN) return 0;
...
dirent_t *mem_readdir(uint32 hnd)
{
	...
	slen = strlen(dir->name);
	for(i=dir->idx;i<tblcnt && (tbl[i].name[0]==0 || strncmp(dir->name,tbl[i].name,slen));i++) ;
	dir->idx = i+1;
	...
	dir->dirent.time = 0;
	dir->dirent.attr = 0;
	strcpy(dir->dirent.name,tbl[i].name+slen);
